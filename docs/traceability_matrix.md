...
|---|---|---|---|---|---|
| Singapore weather ingestion / QC | `docs/singapore_timeseries_input_contract.md`; `data/schemas/singapore_weather_v1.toml` | `src/weather.rs` | source-level unit tests plus successful Rust execution-evidence runs #12 and #13 for typed metadata/provider flags and missing-data semantics | canonical measured Singapore time series not yet acquired; manifest/licence/QC gates remain open | Foundation parser/QC only; strict schema-v1 timestamps normalize to absolute UTC, interval/gap and non-adjacent duplicate diagnostics exist, optional humidity/pressure and provider flags are preserved, typed provider/site/licence metadata exists, and rejected rows are separated from temporal gaps; actual manifest-file binding, canonical measured data and closure diagnostics remain incomplete |
