...
|---|---|---|---|---|---|
| Singapore weather ingestion / QC | `docs/singapore_timeseries_input_contract.md`; `data/schemas/singapore_weather_v1.toml` | `src/weather.rs` | source-level unit tests exist; qualifying `cargo test` execution evidence still required | canonical measured Singapore time series not yet acquired; manifest/licence/QC gates remain open | Foundation parser/QC only; strict schema-v1 timestamps normalize to absolute UTC and interval/gap plus non-adjacent duplicate diagnostics now exist; provider flags, manifest/schema parity, closure diagnostics and execution evidence remain incomplete |
