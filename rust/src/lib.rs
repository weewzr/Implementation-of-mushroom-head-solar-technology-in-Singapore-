...
impl Triangle {
    pub fn centroid(&self) -> Vec3 {
        self.v[0].add(self.v[1]).add(self.v[2]).mul(1.0 / 3.0)
    }
    pub fn cross(&self) -> Vec3 {
        self.v[1].sub(self.v[0]).cross(self.v[2].sub(self.v[0]))
    }
...
pub fn direct_visibility(index: usize, triangles: &[Triangle], sun: Vec3) -> f64 {
    let s = sun.unit();
    let origin = triangles[index]
        .centroid()
        .add(s.mul(EPS * RAY_OFFSET_MULTIPLIER));
    for (j, blocker) in triangles.iter().enumerate() {
        if j != index && ray_triangle_distance(origin, s, blocker).is_some() {
            return 0.0;
        }
    }
    1.0
...
pub fn array_direct_projected_area(triangles: &[Triangle], sun: Vec3) -> f64 {
    ...
    triangles
        ...
        .map(|(i, t)| {
            ...
            if mu > 0.0 {
                t.area() * mu * direct_visibility(i, triangles, s)
            } else {
                0.0
            }
        })
